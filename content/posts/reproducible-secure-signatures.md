...
The user, as verifying party, requests the remote attestation of a confidential environment, receiving information about all applications within the environment.
Next, the verifying party audits the source code of the application.
Lastly, the verifying party must connect the audited source code to the information in the remote attestation.
...
![Structure of ID block and attestation report](/img/idblock.svg)
...
## Attesting the signing environment
...
In step 2, there might be multiple points that satisfy the curve equation, meaning we recover only a candidate in step 5.
This can be made deterministic by always selecting the point with the smallest $y_R$ in step 2.
...
Rust implements this algorithm in its [standard library].
Since Go doesn't, we [implement it ourselves].
...
[implement it ourselves]: https://github.com/edgelesssys/contrast/blob/45f1b5edde0bc3dc16201063660613db3168519a/internal/idblock/idblock.go#L225
[standard library]: https://docs.rs/ecdsa/0.16.9/ecdsa/struct.VerifyingKey.html#method.recover_from_msg
...
Nobody knows the private key $d$ for the recovered public key $B$, and computing it from $B$ is believed infeasible under the elliptic curve discrete logarithm assumption.
To forge a valid signature for a different message (like a malicious, attacker-crafted ID block) under $B$, an attacker would need to either learn the discrete log of $B$, or find another message that yields the same recovered public key, which amounts to a second-preimage attack on the hash.
...
*To my co-author [Leonard Cohnen], who worked together with me on the implementation of this feature.
To Thomas Tendyck who came up with the idea of using ECDSA key recovery.
And to Edgeless Systems, for giving me the opportunity to work on this.*

[Leonard Cohnen]: https://infosec.exchange/@Euler
